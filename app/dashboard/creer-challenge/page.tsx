...
export default function CreerChallengePage() {
    ...
    const handleFormAction = async (formData: FormData) => {
        // Validation côté client des règles
        const reglesValides = regles.filter(r => r.trim()).length > 0;

        if (!reglesValides) {
            toast.error("Veuillez ajouter au moins une règle valide");
            return;
        }
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-black rounded-3xl p-8 shadow-2xl shadow-white/10">
                <form action={handleFormAction} className="space-y-8">
                    ...
                    <div>
                        <label className="block text-white font-bold mb-3 text-sm uppercase tracking-wide font-sans">
                            Pourcentage de vote du créateur *
                        </label>
                        <input
                            name="pourcentageVote"
                            type="number"
                            min="1"
                            max="100"
                            required
                        ...
                        <p className="text-xs text-gray-400 mt-2 font-sans">
                            💡 Le pourcentage est souvent 100% au lancement, mais tu peux le modifier si besoin.
                        </p>
                    </div>
                    ...
                    <div>
                            ...
                            {regles.map((regle, index) => (
                                <div key={index} className="flex items-start gap-3 group">
                                    ...
                                    <input
                                        ...
                                        className="flex-1 bg-white border-2 border-black rounded-2xl px-5 py-3 text-black placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-black/10 transition-all font-sans" // ⬅️ ENLEVÉ "required"
                        ...
                        <div className="mt-3">
                            ...
                            {/* Message d'erreur si aucune règle valide */}
                            {regles.filter(r => r.trim()).length === 0 && (
                                <p className="text-red-400 text-sm font-sans mt-2">
                                    ⚠️ Au moins une règle doit être remplie
                                </p>
                            )}
                            ...
                            <p className="text-xs text-gray-400 mt-2 font-sans">
                                ...
                                <span className="text-green-400">
                                    Règles à respecter : {regles.filter(r => r.trim()).length}
                                </span>
                            </p>
                        </div>
                    </div>
